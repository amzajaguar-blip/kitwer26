  ...
  return (
    <footer className="w-full mt-16 sm:mt-24 border-t border-zinc-800 py-10 px-6 bg-zinc-950">
      ...
      <nav className="flex flex-wrap justify-center gap-x-4 gap-y-2 mb-6">
        {[
          ...
          <Link
            key={href}
            href={href}
            className="font-mono text-[10px] tracking-widest text-zinc-600 hover:text-cyan-400 transition-colors uppercase"
          >
            {label}
          </Link>
        ))}
      ...
      <div className="border-t border-zinc-800 pt-5">
        <p className="font-mono text-[10px] text-center text-zinc-700 tracking-widest uppercase mb-2">
          © {new Date().getFullYear()} Kitwer26 · Tutti i diritti riservati
        </p>
        <p className="font-mono text-[9px] text-center max-w-xl mx-auto leading-relaxed text-zinc-700">
          In qualità di Affiliato Amazon, Kitwer26 riceve una commissione sugli acquisti idonei effettuati tramite i link presenti sul sito.
          I prezzi possono variare.{' '}
          <Link href="/legal" className="underline hover:text-cyan-400 transition-colors">
