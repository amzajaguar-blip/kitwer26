[TASK]
Agisci come un Senior Developer / System Architect autonomo. Il tuo compito è automatizzare e migliorare il flusso di acquisto dell'e-commerce Kitwer26, integrando la gestione dei link di affiliazione Amazon per i prodotti e ottimizzando le notifiche e la gestione lato amministratore. Questo include la persistenza dei dati dei prodotti nel database, l'estensione del pannello admin e l'automazione delle comunicazioni post-acquisto, seguendo il flusso dettagliato fornito.

[CONTEXT FILES]
Il codice base dell'e-commerce Kitwer26 è stato fornito in precedenza e analizzato. I file chiave includono:
- `/home/ubuntu/kitwer26-ecommerce/shared/products.ts`: Definizione attuale dei prodotti (statici).
- `/home/ubuntu/kitwer26-ecommerce/drizzle/schema.ts`: Schema del database (solo `users` e `orders`).
- `/home/ubuntu/kitwer26-ecommerce/server/db.ts`: Funzioni di accesso al database (solo `users` e `orders`).
- `/home/ubuntu/kitwer26-ecommerce/client/src/pages/Admin.tsx`: Pannello di amministrazione (solo gestione ordini).
- `/home/ubuntu/kitwer26-ecommerce/server/routers.ts`: Definizione delle API tRPC (prodotti di sola lettura).
- `/home/ubuntu/kitwer26-ecommerce/client/src/pages/Home.tsx`: Pagina principale del catalogo prodotti.
- `/home/ubuntu/upload/pasted_content.txt`: Flusso di acquisto desiderato.

[REFERENCE]
Il design e la logica devono seguire i pattern del progetto Kitwer26 (stile dark/cyberpunk, Tailwind CSS, React, tRPC, Drizzle ORM, MySQL, logica d'élite). In particolare, il pannello admin deve mantenere la coerenza con l'interfaccia esistente per la gestione degli ordini.

[SUCCESS BRIEF]
Il task si considera concluso con successo SOLO quando il codice/sistema fornito è completo, privo di bug evidenti e pronto per essere copiato in produzione senza ulteriori modifiche da parte mia. Questo include:
- Tabella `products` nel database con campo `amazonLink`.
- Funzioni CRUD complete per i prodotti in `server/db.ts`.
- Procedure tRPC `admin.products` per la gestione completa dei prodotti.
- Interfaccia utente `AdminProducts.tsx` integrata nel pannello admin per la gestione dei prodotti (creazione, modifica, eliminazione, visualizzazione `amazonLink`).
- Visualizzazione dei link Amazon nella `Home.tsx` se presenti.
- Logica del webhook Stripe (`/api/webhooks/stripe`) modificata per recuperare e includere i `amazonLink` nelle email di notifica admin e generare ricevute PDF con QR code.
- Tutte le modifiche devono essere coerenti con l'architettura esistente e le best practice di sicurezza.

[RULES: RAGIONAMENTO AUTONOMO RICORSIVO]
Da questo momento, devi operare con questa logica rigorosa:

    Identifica problemi complessi: Analizza la mia richiesta e prevedi i bug o i colli di bottiglia.

    Genera soluzioni: Pensa a più approcci e scegli il migliore per performance e scalabilità.

    Implementa & Testa: Scrivi il codice simulando un test mentale rigoroso.

    Autocorrezione Ciclica: Se durante la stesura ti accorgi che un pezzo di logica fallisce, NON fermati per chiedermi cosa fare. Correggilo in autonomia spiegandomi dopo cosa hai fixato.

    Elimina "Human-in-the-loop": Non chiedermi mai "Vuoi che proceda?", "Vuoi che ti scriva il codice?", "Cosa ne pensi?". Esegui il lavoro fino al completamento totale del Success Brief.

[CONVERSATION PLAN & ALIGNMENT]
Prima di generare l'output finale, scrivi un brevissimo elenco puntato (max 3 righe) del piano d'azione che stai per eseguire. Subito dopo, senza attendere il mio via libera, procedi con l'esecuzione completa del codice.

## Flusso di Acquisto Desiderato (Master Maxx)
...
*   Salva ordine su MySQL/Drizzle (status: `pending_stripe_payment`).
...
## Task Dettagliati per l'Implementazione
